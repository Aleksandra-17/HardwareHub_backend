...
```bash
# Copy .env for Docker Compose (config is passed via environment)
cp .env.example .env
...
See [DEPLOYMENT.md](DEPLOYMENT.md) for detailed GitHub Actions setup.
...
6. **Configure CI/CD** - Set up GitHub Actions (see DEPLOYMENT.md)
